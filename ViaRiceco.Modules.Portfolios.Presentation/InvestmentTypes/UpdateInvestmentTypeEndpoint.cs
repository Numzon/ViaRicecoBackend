...
using ViaRiceco.Modules.Portfolios.Application.InvestmentTypes.UpdateInvestmentStrategyType;
...
internal sealed class UpdateInvestmentTypeEndpoint(ISender sender, IHyperlinkService hyperlinkService, IDataShapingService dataShapingService)
    : Ep.Req<UpdateInvestmentTypeEndpoint.Request>.Res<Result<InvestmentStrategyTypeDto>>
{
    [UsedImplicitly]
    ...
    {
        Put("/portfolios/investment-types/{id}");
        Tags(EndpointTags.InvestmentTypes);
        AllowAnonymous();
        Description(d => d.WithName(nameof(UpdateInvestmentTypeEndpoint)));
