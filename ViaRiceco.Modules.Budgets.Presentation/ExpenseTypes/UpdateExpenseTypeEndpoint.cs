...
internal sealed class UpdateExpenseTypeEndpoint(ISender sender, IHyperlinkService hyperlinkService, IDataShapingService dataShapingService)
    : Ep.Req<UpdateExpenseTypeEndpoint.Request>.Res<Result<ExpenseTypeDto>>
{
    [UsedImplicitly]
    ...
    {
        Put("/budgets/expense-types/{id}");
        AllowAnonymous();
        Description(d => d.WithName(nameof(UpdateExpenseTypeEndpoint)));
    ...
    {
        var command = new UpdateExpenseTypeCommand(req.Id, req.Name);
