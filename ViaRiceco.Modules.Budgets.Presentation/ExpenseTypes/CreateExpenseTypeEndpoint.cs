...
internal sealed class CreateExpenseTypeEndpoint(ISender sender, IHyperlinkService hyperlinkService, IDataShapingService dataShapingService)
    : Ep.Req<CreateExpenseTypeEndpoint.Request>.Res<Result<ExpenseTypeDto>>
{
    [UsedImplicitly]
    ...
    {
        Post("/budgets/expense-types");
        AllowAnonymous();
        Description(d => d.WithName(nameof(CreateExpenseTypeEndpoint)));
    ...
    {
        var command = new CreateExpenseTypeCommand(req.Name);
        Result<ExpenseTypeDto> result = await sender.Send(command, ct);
        ...
        await Send.ResultAsync(Results.CreatedAtRoute(
            nameof(GetExpenseTypeEndpoint), 
            new { id = result.Value.Id },   
            shapedObject));
    }

    private ExpandoObject ShapeDataWithConditionalLinks(ExpenseTypeDto data, bool includeLinks, string expenseTypeId, string? fields = null)
    {
        return includeLinks
            ? dataShapingService.ShapeData(data, fields, ExpenseTypesHyperlinks.CreateExpenseTypeItemLinks(hyperlinkService, expenseTypeId))
